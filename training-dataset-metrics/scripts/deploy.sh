...
echo "Pushing training_dataset_metrics:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(training_dataset_metrics:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push training_dataset_metrics:latest to Roboto's private registry"
    exit 1
fi

echo "Creating/updating training_dataset_metrics action"
create_args=(
