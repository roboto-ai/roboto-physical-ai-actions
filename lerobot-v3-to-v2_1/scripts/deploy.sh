...
echo "Pushing lerobot_v3_to_v2_1:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(lerobot_v3_to_v2_1:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push lerobot_v3_to_v2_1:latest to Roboto's private registry"
    exit 1
fi

echo "Creating/updating lerobot_v3_to_v2_1 action"
create_args=(
